...
describe('<Movie/>', ()=>{
    ...
    it('should filter the movie list when the user types into the search input', ()=>{
        ...
        const { queryByText, getByPlaceholderText } = render(<Movie movies={movies}/>);
        ...
        expect(queryByText(new RegExp(movies[0].title, "i"))).toBeInTheDocument()
        expect(queryByText(new RegExp(movies[1].title, "i"))).not.toBeInTheDocument()
        expect(queryByText(new RegExp(movies[2].title, "i"))).not.toBeInTheDocument()
